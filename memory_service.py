...
from uagents import Agent, Context  # import SDK
...
AGENTVERSE_API_KEY = 'ak_live_eyJhbGciOiJSUzI1NiJ9.eyJleHAiOjE3NzMxMTkwNTIsImlhdCI6MTc3MDUyNzA1MiwiaXNzIjoiZmV0Y2guYWkiLCJqdGkiOiI3ZjI1ZThiZTI4ZmZjYWUwYTI5M2M5M2YiLCJzY29wZSI6ImF2Iiwic3ViIjoiYzFjYzVmMWZmM2M0ZTc0OGY4NzEwZjMyYmMyN2U4YTBhOGY3OTJjYjViYjcxNTVkIn0.O0KLxrgi1LOUQOLw5avgt2xyRCThSlRiei6PErteqh6q2GOQXAGTDXWqSEv9OL2dR-_VksrZZ441auYb4tlM7i65sS2gestP3LuD0X32APzdMPvZsjpPZu2zuOLCsTzaDSFQq35NebWiQvY3WqYB_H_4w4iSGqOl34uAFb8x9D_b8H6hcjiuUtf5xCyXar2WUVx-UdOYbmdLezotx9qGZqwFseXdyvnhv6OPxR47-rqrmaa-yN-GYwFW9e4MMD4gwEAUqJ98A0ZsMDHeugkkykjDRZ9Pm-QmEfu0eg2fXy40vtmkoini59tGeS_dyXToi2mUsN-lERA4oweqg2LJrQ'  # کلید واقعی
...
def subscribe():
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute('UPDATE api_keys SET subscribed = 1 WHERE key = ?', (api_key,))
                conn.commit()
...
def store_memory():
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute('INSERT INTO memories (agent_id, data) VALUES (?, ?)', (user['agent_id'], encrypted))
                conn.commit()
...
def register_to_agentverse():
    ...
    try:
        # ایجاد agent با SDK uagents
        agent = Agent(name=f"{BRAND_NAME} Memory Provider", seed=user['agent_id'])
        agent.register()  # ثبت در Agentverse
        status = {'status': 'registered', 'agent_address': agent.address}
